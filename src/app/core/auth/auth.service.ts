...
export class AuthService {
  ...
  public readonly tokenClients = computed<ContextClient[]>(() => {
    ...
    if (!rawClients) {
      ...
      if (rawCids) {
        ...
        return ids.map((id, index) => ({
          id,
          name: ids.length === 1 ? String(singleName) : `Client ${index + 1} (${id.slice(0, 8)})`,
        }));
      }
      return [];
    }
    ...
    return parsedList
      .map((item) => {
        ...
        const id = String(item.ClientId ?? item.clientId ?? item.client_ids ?? item.id ?? item.Id ?? '');
  ...
  /**
   * Extracts available sites list from token claims if provided by backend.
  ...
  private initContextFromToken(claims: any = this.userClaims()): void {
    ...
    if (!this.selectedClientId()) {
      ...
      if (cid) {
        const idStr = Array.isArray(cid) ? String(cid[0]) : String(cid).split(',')[0].trim();
        if (idStr) {
    ...
    if (!this.selectedSiteId()) {
      ...
      if (sid) {
        const idStr = Array.isArray(sid) ? String(sid[0]) : String(sid).split(',')[0].trim();
        if (idStr) {
